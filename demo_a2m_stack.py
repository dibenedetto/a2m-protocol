"""The whole memory stack, over A2M, on disk.
	...
	python demo_a2m_stack.py            # single server, per-tier SQLite
	python demo_a2m_stack.py --router   # federated: one process per tier
...
from   jsonrpc   import Client, LocalTransport
...
def open_single(path: pathlib.Path) -> tuple[MemoryClient, Any]:
	...
	from a2m_store import open_stack
...
def open_router(root: pathlib.Path) -> tuple[MemoryClient, Any]:
	from a2m_router import MemoryRouter, RankFusion, spawn_backends
...
def demo_persistence(federated: bool) -> None:
	...
	writer = (
		...
		"from demo_a2m_stack import open_single, ROOT, TIERS;"
